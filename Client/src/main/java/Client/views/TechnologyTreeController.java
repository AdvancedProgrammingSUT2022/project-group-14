...
public class TechnologyTreeController {
    ...
    public void initTechnologies() {
        ...
        while (downLayers.size() > 0) {
            ...
            currentLayer = downLayer;
            downLayer = new ArrayList<>();
            addLayerToGroup(currentLayer);
    ...
    public void searchButtonClicked(MouseEvent mouseEvent) {
        for (Node hBoxChild : hBox.getChildren()) {
            for (Node child : ((VBox) hBoxChild).getChildren()) {
                if (((ImagePattern) ((Circle) ((Group) child).getChildren().get(5)).getFill()).getImage().getUrl().contains(searchField.getText())){
                    scrollPane.setHvalue((child.getLayoutX() + hBoxChild.getLayoutX()) / anchorPane.getWidth() * 1.1);
                }
            }
        }
    }
