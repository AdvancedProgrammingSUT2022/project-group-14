...
public class Tile {
    ...
    public static Tile generateRandomTile(int x, int y) {
        ...
        Response response = ClientSocketController.sendRequestAndGetResponse(QueryRequests.GET_MAP_RANGES, new HashMap<>());
        Pair<String, String> ranges = new Pair<>(response.getParams().get("width"), response.getParams().get("height"));
        if (x == 0 || x == Integer.parseInt(ranges.getValue()) - 1 || (y == 0 && x % 2 == 0) || (y == Integer.parseInt(ranges.getKey()) - 1 && x % 2 == 1)) {
...
//        }
    ...
    public void addResourceToCivilizationAndTile(Resource resource) {
        ...
        if (resource.getType() instanceof LuxuryResourceTypes) {
            ClientSocketController.sendRequestAndGetResponse(QueryRequests.SET_LUXURY_RESOURCE, new HashMap<>() {{
                put("name", resource.getName());
            }});
        } else if (resource instanceof StrategicResource) {
            ClientSocketController.sendRequestAndGetResponse(QueryRequests.SET_STRATEGIC_RESOURCE, new HashMap<>() {{
                put("name", resource.getName());
            }});
        }
    }
